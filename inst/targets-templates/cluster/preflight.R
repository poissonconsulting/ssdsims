...
#   * `dqrng` loads on the worker          (else: install it alongside ssdsims),
...
ssdsims_cluster_probe <- function(expected = NULL) {
  ...
  # (2b) dqrng loads on the worker. ssdsims uses dqrng for reproducible
  # per-task seeding but never loads it itself (a Suggested dependency), so a
  # shard aborts without it — catch a missing worker install here, not mid-run.
  if (!requireNamespace("dqrng", quietly = TRUE)) {
    stop(
      "Cluster probe FAILED [dqrng unavailable]: the worker on node '",
      node,
      "' cannot load `dqrng` (>= 0.4.1), which ssdsims requires for the ",
      "per-task RNG backend. Install it on the worker via the same ManyLinux ",
      "binary path as ssdsims (see this template's README, backend B).",
      call. = FALSE
    )
  }
...
message(
  ...
  "' running R ",
  witness$r_version,
  ", ssdsims + dqrng load, scratch writable."
)
