...
Derive the four per-step task tables (`sample`, `data`, `fit`, `hc`) from an `ssdsims_scenario` — the flat per-task data shape the rest of the targets-based pipeline builds on (`TARGETS-DESIGN.md` §1–§2, §5). Splitting the random draw (`sample`) from its truncation (`data`) keeps the §5 sub-truncation property structural (one draw shared across every `nrow`) while letting `nrow` be a clean cross-join axis downstream. Each table carries a path-style `<step>_id` primary key and its parent's id as a foreign key, is a printable `ssdsims_tasks` classed tibble, and is bundled by the canonical `ssd_scenario_tasks()` expansion. A `purrr::pmap()` baseline runner executes the tables in dependency order with no RNG seeding, no `targets`, no shards, and no Parquet I/O.
