# cluster-pipeline Specification

## Purpose

Ship an editable cluster `targets` template (`inst/targets-templates/cluster/`) that runs a scenario's whole pipeline over a SLURM queue via `crew.cluster::crew_controller_slurm()`, reusing the `ssd_scenario_targets()` factory and scenario shape verbatim from the `large/` template. A standalone preflight validates connectivity and worker prerequisites before any scenario logic runs; shards are the unit of parallelism dispatched to SLURM jobs; and a zero-to-running-job guide ships as a vignette and the template README (`TARGETS-DESIGN.md` §4/§11).
