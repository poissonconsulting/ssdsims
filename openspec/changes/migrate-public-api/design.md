...
This change is the `migrate-public-api` step: a **cosmetic** carry-over of those three functions onto the dqrng + primer contract, with the L'Ecuyer helpers kept as a one-release shim. It depends on `primer-primitives` (the contract) and on `scenario-input-types` (so its byte-equivalence re-run can exercise the full public input surface), and it **gates `cleanup-lecuyer`** — the L'Ecuyer helpers and `_seed` shims cannot be removed until the public step functions stop calling them. The benchmark of correctness is the loop-free `scripts/example-expanded.R`, which currently proves the L'Ecuyer public path byte-equals a hand expansion built from the `*_state` primitives; after migration it must prove the same equality on the dqrng primitives.
...
- **hc**: parent `(stream, sim)` extended with the hc-grid row (`nboot`, `ci_method`, `parametric`); the scalar `ci` flag and the `est_method` setting are applied uniformly and excluded from the identity (D6).
...
### D6 — `ci` and `est_method` are excluded from the hc per-task primer (reconcile with `scalar-ci-flag` and `est-method-setting`)
The archived `scalar-ci-flag` change demoted `ci` from a grid/task axis to a scenario-wide scalar **scenario setting**, removing it from `task_axes("hc")` and the per-task identity. Its cross-reference decision pinned that *whichever of the two changes lands second drops `ci` from the hc primer enumeration*; `scalar-ci-flag` is now archived, so this change is second and drops it. The archived `est-method-setting` change likewise reclassified `est_method` as an hc scenario setting and removed it from `task_axes("hc")` (every requested `est_method` is summarised from a cell's single bootstrap sample set), so `est_method` SHALL also be dropped from the hc primer enumeration. The hc primer therefore keys on `(stream, sim, nboot, ci_method, parametric)` only — `est` is invariant to `ci`, and `est_method` no longer fans out into separate tasks, so neither carries task-distinguishing information and a constant term would only obscure the hash. `ssd_hc_sims()` already applies `ci` as a scalar pass-through (no factorial expansion over it, and no `ci = FALSE`-to-`NA` "collapse" — that machinery lived in the declarative `hc_grid_tbl()` and was retired by `scalar-ci-flag`) and, post-`est-method-setting`, no longer fans out over `est_method`, so migrating its seeding requires only that `ci` and `est_method` stay out of the primer identity.
