...
- Specify the canonical, name-keyed hash input per RNG-consuming step of #80's four-step model (`sample`: `(dataset, sim, replace)` — the draw, no `nrow`; `fit`: parent identity + fit-grid row; `hc`: parent identity + hc-grid row including `ci`), with function-valued parameters (`min_pmix`) referenced **by name** so a recompile/JIT does not move a task's primer. The `data` truncation step is RNG-free (`head(sample, nrow)`) and takes no primer. Building the actual task tables is `task-list-loop-baseline` (#80) / `task-tables`; this change owns only the primer function and its input contract.
