...
- Specify the canonical, name-keyed hash input per step (data: `(dataset, sim, replace)`; fit: data identity + fit-grid row; hc: fit identity + hc-grid row), with function-valued parameters (`min_pmix`) referenced **by name** so a recompile/JIT does not move a task's primer. Building the actual task tables is out of scope (`task-list-loop-baseline`/`task-tables`); this change owns only the primer function and its input contract.
