...
### Decision: Parquet backend is `arrow` (with `nanoparquet` as the considered alternative)

`TARGETS-DESIGN.md` writes `arrow::read_parquet()` and a `duckplyr` summary layer, so `arrow` is the faithful default and pairs with the predicate-pushdown query layer `hive-partitioning`/`summary` will need. *Alternative considered:* `nanoparquet` — zero-system-dependency, CRAN, lighter to install on cluster nodes, and sufficient for the simple one-file-per-dataset write/read here. The trade-off is real (`arrow` is a heavy build; the crew labs already pinned a ManyLinux binary path, §4, which mitigates it). Decision: **`arrow`**, matching the design; revisit if cluster install cost bites, since the registry's read/write surface is small enough to swap behind `ssd_read_parquet()` / `ssd_write_parquet()` internals.
...
- **`arrow` is a heavy dependency** → mitigated by the crew labs' ManyLinux binary install path (§4) and by confining Parquet I/O behind thin `ssd_read_parquet()`/`ssd_write_parquet()` internals so `nanoparquet` can be swapped in without touching callers.
