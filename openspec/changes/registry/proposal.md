...
- Introduce the Parquet backend (`arrow`) and a shared file-sha256 helper.
...
- **Dependencies**: adds `arrow` (Parquet I/O) and `digest` (file sha256) to `Imports`. (See design for the `arrow` vs `nanoparquet` trade-off.)
