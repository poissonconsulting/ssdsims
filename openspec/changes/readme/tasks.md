...
- [ ] 1.5 Add a short declarative-scenario teaser using `ssd_scenario_data()` / `ssd_define_scenario()` / `ssd_scenario_tasks()`; evaluate it if it stays fast and deterministic, otherwise set `eval = FALSE` and rely on the article link.
- [ ] 1.6 Add a compact capability map (immediate pipeline; scenarios & task expansion; targets pipeline; designs that combine scenarios; cloud upload; cost estimation/analysis; reproducible RNG) linking to the pkgdown articles `defining-a-scenario`, `sharded-pipeline`, `scenario-to-design`, `cluster-pipeline`, `cloud-upload`, `cost-estimation`, and `cost-analysis`.
