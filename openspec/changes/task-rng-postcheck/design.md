...
`TARGETS-DESIGN.md` §2 roots every per-task draw in `dqrng::dqset.seed(seed, stream = primer)` under an active `local_dqrng_backend()` (pcg64) scope. `primer-primitives` lands the per-task seed-and-run wrappers (`*_data_task_primer()`) that install the primer once and run the operation. The entry side is guarded: `local_dqrng_state()` calls `chk_dqrng_backend_active()` before seeding. There is no exit side — nothing certifies the backend was *still* dqrng when the task finished drawing.
