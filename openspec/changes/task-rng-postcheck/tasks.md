...
- [x] 3.2 Foreign-hijack scenario: **descoped to the exploration reprexes** (`exploration/.../case6-witness-vs-hijack.R`) rather than a live test. A live test would require co-loading a second user-RNG package (`randtoolbox`) in the test process — itself the process-global, potentially crashing act this change exists to avoid — and `skip_if_not_installed("randtoolbox")` would *load* it. So `randtoolbox` is not a test dependency; case6 is the reference (per PR review).
- [x] 3.3 Torn-down test: `chk_dqrng_backend_intact()` aborts when dqrng is not the active backend (no user-supplied RNG driving), reporting the current `RNGkind()` and naming no symbol owner — exercised in-process without activating-then-corrupting the backend (which is unsafe in a shared process)
- [x] 3.3a Message-content for a foreign hijack (owner + provider list, `rng_slot_owner()` / `user_rng_providers()`): **descoped to `exploration/.../case7-who-owns-rng.R`** for the same reason as 3.2 (per PR review); the code path remains in `R/dqrng-backend.R`
...
- [x] 3.5 Primitives test: each `*_data_task_primer()` returns normally on a healthy backend (the postcondition's failure path is covered by the witness tests 3.1/3.3 + the exploration reprexes, not by corrupting a live shared process)
- [x] 3.6a Conditional-dependency tests: `dqrng_usable()` is `TRUE` and activation succeeds when dqrng is loaded; and `local_dqrng_backend()` aborts with the `library(dqrng)` guidance when dqrng is not usable (exercised in-process by forcing the gate, as dqrng cannot be cleanly unloaded mid-session; no `callr` dependency)
