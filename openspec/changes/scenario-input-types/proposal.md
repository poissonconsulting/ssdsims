...
- **Downstream**: A generated dataset is a realised tibble on the scenario, so `task-list-loop-baseline` runs it directly. The later `registry` step adds Parquet persistence; `dataset-provenance` (§12, deferred) adds name-only regeneration and provenance metadata. The `ssd_run_scenario()` legacy path is untouched.
