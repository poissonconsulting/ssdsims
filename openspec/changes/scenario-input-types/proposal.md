...
- **Depend on `task-rng-postcheck` (PR #117).** `ssd_gen()` gates every dqrng touch behind `dqrng_usable()` (use dqrng only when the caller has already loaded it; abort with actionable `library(dqrng)` guidance otherwise) and uses `chk_dqrng_backend_intact()` as the post-hoc witness that each draw actually came from dqrng — reusing #117's shared, tested machinery instead of a bespoke RNG-state check. With `.seed` required, the witness reduces to "drew from dqrng (reproducible) or pure (reproducible) → ok; escaped dqrng → abort".
