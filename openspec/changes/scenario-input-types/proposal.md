...
- **Depend on `task-rng-postcheck`.** `ssd_gen()` gates every dqrng touch behind `dqrng_usable()` (use dqrng only when the caller has already loaded it; abort with actionable `library(dqrng)` guidance otherwise) and uses `chk_dqrng_backend_intact()` as the post-hoc witness that each draw actually came from dqrng — reusing `task-rng-postcheck`'s shared, tested machinery instead of a bespoke RNG-state check. With `.seed` required, the witness reduces to "drew from dqrng (reproducible) or pure (reproducible) → ok; escaped dqrng → abort".
...
- **Dependencies**: depends on `task-rng-postcheck` (`dqrng_usable()`, `chk_dqrng_backend_intact()`, dqrng moved to a Suggests) — booked under Done in `ROADMAP.md` but **not yet physically in the tree**, so it must land before §3 here — plus `task-primer`/`local-dqrng-state` for scoped name-streamed generation, in addition to `rlang` (symbol capture, splicing) and `chk` (validation). The dqrng/`task_primer` edge lives in `ssd_gen()` only — the scenario constructor keeps the inert, RNG-free contract `ssd-define-scenario` established.
