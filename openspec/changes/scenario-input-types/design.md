...
- **Heterogeneous collection complicates downstream consumers** → `task-list-loop-baseline`/`task-tables` and the registry must handle both element types. Mitigation: the classed descriptor gives a clean `inherits(x, "ssdsims_generator")` discriminator, and the dataset list is keyed by name so consumers iterate names uniformly; only materialisation differs.
