...
- **THEN** the expansion SHALL produce one row per `est_method` with `nboot`/`ci_method`/`parametric` set to `NA`, rather than fanning out across those bootstrap-only knobs
