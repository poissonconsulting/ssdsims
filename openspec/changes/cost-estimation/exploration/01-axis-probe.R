...
t_p4 <- tt(ssd_hc(fit, proportion = c(0.01, 0.05, 0.1, 0.2), ci = TRUE, nboot = nb, min_pboot = 0))
cat(sprintf("proportion x1 = %.2fs  x4 = %.2fs  (ratio %.2f -> FREE)\n", t_p1, t_p4, t_p4 / t_p1))

# ci_method: per-method cost (the dominant axis)
t_ci <- vapply(ssd_ci_methods(), \(m) tt(ssd_hc(fit, ci = TRUE, nboot = nb, ci_method = m, min_pboot = 0)), numeric(1))
...
t_est <- vapply(ssd_est_methods(), \(m) tt(ssd_hc(fit, ci = TRUE, nboot = nb, est_method = m, min_pboot = 0)), numeric(1))
cat("\nper-est_method time (all ~equal -> FREE):\n")
print(round(t_est, 2))

# samples retention: free
t_sampF <- tt(ssd_hc(fit, ci = TRUE, nboot = nb, samples = FALSE, min_pboot = 0))
