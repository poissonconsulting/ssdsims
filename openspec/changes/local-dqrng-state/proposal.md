...
- **Backend guard.** `local_dqrng_state()` SHALL check the active RNG kind and **abort** if the dqrng backend is not in effect — i.e. if `RNGkind()[1] != "user-supplied"` (the signal that `dqrng::register_methods()` / `local_dqrng_backend()` is active). The error directs the caller to open a `local_dqrng_backend()` scope first. This makes the dependency on the active backend explicit and fail-fast rather than silently seeding the wrong generator.
