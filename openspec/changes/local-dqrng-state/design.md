...
- Guard on the active backend: abort if `RNGkind()[1] != "user-supplied"`, i.e. if no `local_dqrng_backend()` scope is open.
...
- **Decision: guard on `RNGkind()`, aborting outside an active backend.** `local_dqrng_state()` is only meaningful while `local_dqrng_backend()` (from `dqrng-init`) has registered dqrng's methods. It checks `RNGkind()[1] == "user-supplied"` and aborts with an actionable message otherwise. This fails fast instead of silently seeding base R's Mersenne-Twister (which `dqset.seed()` would not even touch), and makes the backend dependency explicit at the call site. *Alternative*: silently activate the backend itself — rejected; that would hide the missing `local_dqrng_backend()` scope and muddy who owns backend lifetime. *Alternative*: check via `dqRNGkind()` — rejected; dqrng 0.4.1 offers no getter, so `RNGkind()` is the observable signal.
