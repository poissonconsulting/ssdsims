...
- [ ] 2.4 Parent-consistency check: for each non-root step, `intersect(path[[step]], task_axes(parent))` SHALL be a subset of `path[[parent]]` (chain `sample ← fit ← hc` via `task_parent()`); abort otherwise
...
- [ ] 5.1 Update `ssd_define_scenario()` roxygen `@param partition_by` to document the three-step named-list contract, the per-step vocabularies (reused from `task_axes()`), the `sample`-only `nrow` rejection, the parent-consistency rule, and the all-or-nothing override
