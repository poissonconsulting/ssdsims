...
- [ ] 5.1 Add an internal predicate `dqrng_backend_active()` returning `identical(RNGkind()[1], "user-supplied")` (the observable signal that `register_methods()` is in effect; dqrng 0.4.1 exposes no `dqRNGkind()` getter)
- [ ] 5.2 Make `local_dqrng_backend()` reentrant: if `dqrng_backend_active()`, return a no-op invisibly (no re-`set`, no deferred `reset`); only the outermost scope registers on entry and resets on exit
- [ ] 5.3 Reentrancy test in `test-dqrng-backend.R`: a nested `local_dqrng_backend()` call is a no-op and the draw sequence is **identical with vs. without** the nested call; outer backend stays active until the outermost scope exits
- [ ] 5.4 Note the reentrant nesting contract in the docs and CLAUDE.md; re-run `devtools::document()`, `air` formatting, and `devtools::check()`
