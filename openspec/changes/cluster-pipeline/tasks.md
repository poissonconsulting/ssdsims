...
- [ ] 2.3 Add a no-op connectivity probe target that submits and reclaims one SLURM job, wired so `tar_make()` builds it before any scenario shard target (§4 ingredient B — validate wiring before ssdsims logic)
