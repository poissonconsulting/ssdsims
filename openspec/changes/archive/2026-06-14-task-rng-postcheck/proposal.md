...
- Add an internal `chk_dqrng_backend_intact(call = ...)` (a `chk`-style guard in `R/dqrng-backend.R`) that:
...
- The cheap `dqrng_backend_active()` `RNGkind()` predicate is **retained** solely as the reentrancy no-op gate inside `local_dqrng_backend()` (re-asserting `register_methods()` reseeds, so that gate must stay cheap and side-effect-free) — it is not the integrity assertion. Its aborting wrapper `chk_dqrng_backend_active()` was the old entry guard; once the entry guard is upgraded to the witness it has no remaining caller, so it is **removed** as dead code.
