...
- **`replace` field on the scenario**: #80 reads `scenario$replace` (defaulting to `FALSE`). `replace` is in the `sample` vocabulary regardless; if a future `replace` scenario option lands on `ssd_define_scenario()` it flows through unchanged.
