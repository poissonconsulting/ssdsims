...
The reproducibility and completeness contracts in `TARGETS-DESIGN.md` §8.5/§9 rest on a per-scenario **manifest**: a small JSON sidecar recording the scenario's declarative fields, the **complete session info** (R version, platform, and every attached/loaded package version) that guarantees bit-stability across re-runs (§9), and `completed_shards` — each completed shard's partition path plus the sha256 recorded at write time. That sha256 is what lets the lightweight replay recipe verify a locally-regenerated upstream against the cluster's *actual* bytes before re-running a failing task (§7), and what `shard-completeness-assert` reads to record expected-vs-actual per shard (§6.2/§8.4); it is also what `cloud-upload` re-hashes a downloaded copy against to verify a round-trip. No manifest exists today, so none of those downstream guarantees can be built.
...
- Add a per-shard recorder `ssd_record_shard(dir, partition_key, sha256)` — records a shard's trusted-as-produced sha256 **alongside the shard's Parquet** as a small `meta.json` sidecar, one writer per file (no race). This is the *at-write-time* path wired in by the downstream consumers that need the trusted sha (`replay-helper` / `cloud-upload`), **not** the happy-path `task-tables` runner; the baseline assembler instead hashes the shards on disk. The shard directory is **supplied by the caller** (composed from the existing `path_key()` and the step's result root); `manifest` defines no Parquet paths of its own.
- Add a manifest assembler that unions the per-shard sidecars into the manifest's `completed_shards` map (`partition path -> { sha256 }`).
...
- **On-disk layout**: introduces `<results>/manifest.json` and per-shard `meta.json` sha256 sidecars next to each shard Parquet (§8.5).
...
- **Downstream**: feeds `replay-helper` (verify upstream against `completed_shards` sha256 before replay, §7) and `shard-completeness-assert` (record expected-vs-actual per shard, §6.2/§8.4); `cloud-upload` ships the `meta.json` sidecar alongside the shard and verifies a download by re-hashing it against the recorded sha256 (§6.1) — `manifest` carries no cloud-specific field, so the two changes are decoupled. TARGETS-DESIGN.md §12 DAG: `define → manifest`, `manifest → replay-helper`, `manifest → shard-completeness-assert`.
