...
### Decision: re-assembly is idempotent — the loop is write-head-then-assemble-tail

The incremental cycle — run, assemble, **expand the scenario**, run the missing shards, assemble again (`TARGETS-DESIGN.md` §8.6) — needs the assembler to be safe to re-run after new shards appear. It is: the assembler rebuilds `completed_shards` from the Parquets on disk every call and *replaces* the tail wholesale, so a second assembly unions the new shards with the untouched ones and prunes nothing stale — idempotent, and monotone under growth. No incremental-merge bookkeeping is needed (the shards are the truth).

The one ordering constraint: `ssd_write_manifest()` rewrites the whole file with the head alone (the head/tail split above), so it must run *before* the assemble, not after — otherwise it would clobber the freshly assembled tail. After an **expansion** the head must be re-written from the *expanded* scenario so it describes the current grid, then the tail re-assembled from disk. The runner's contract is thus *write-head-then-assemble-tail* on every (re-)run; head and tail are each pure functions of current state, so they cannot drift across runs. *Alternative considered:* make the assembler preserve-and-merge an existing tail incrementally — rejected as needless state; a fresh walk of the tree is cheap and cannot leave a stale entry.
...
- **`jsonlite` numeric/integer fidelity** → use `auto_unbox = TRUE` and read with `simplifyVector = FALSE`; cover integer `seed`/`nboot` and logical scenario options in a round-trip test so coercions are caught.
...
- **Incremental vs. post-run assembly.** Assembling `completed_shards` once after `tar_make()` returns is simplest and matches §6.2's "errors read after the run"; re-running the assembler after an expansion is idempotent (see the re-assembly decision above). An incremental assembler that updates the tail *mid-run* — while shards are still being written — is a later optimisation if manifests are queried before `tar_make()` returns.
