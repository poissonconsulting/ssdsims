...
- [x] 2.2 Add `ssd_read_manifest(dir)`: read with `simplifyVector = FALSE` and restore whole-number/logical scenario option types so the round-trip is lossless
...
- [x] 6.1 `tests/testthat/test-manifest.R`: write/read round-trip preserves declarative fields and the session-info block (incl. the `r`/`dqrng`/`ssdtools` subset), with `seed`/`nboot` whole numbers and logical scenario options intact
