...
- Add a scalar logical `samples` scenario option to `ssd_define_scenario()` (default `FALSE`), stored at `scenario$hc$samples`, validated as a flag, and rendered by `print()` alongside the other hc scenario options.
...
- `scenario-definition`: `ssd_define_scenario()` gains the scalar `samples` scenario option (default `FALSE`), stored on `scenario$hc` and passed through to `ssd_hc()` for output retention only.
