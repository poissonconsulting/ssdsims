...
- Store `min_pmix` **by name** in the `fit` grid (not as a function value), symmetric with datasets: a character vector of names, or a function / list of functions whose name is derived by symbol capture. The registry that resolves a name back to a function is a later step (`registry`); only the name reference lives here.
...
- Apply the `ci = FALSE` rule at construction time: when `ci = FALSE` is the only confidence-interval value, passing any bootstrap-only scenario option (`nboot`, `ci_method`, `parametric`) is **rejected with an error**, forcing the user to omit them or set `ci = c(FALSE, TRUE)`.
