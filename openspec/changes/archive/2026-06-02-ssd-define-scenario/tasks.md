...
- [x] 3.2 Error message names the offending option(s) and points to `ci = c(FALSE, TRUE)` as the way to enable bootstrap
- [x] 3.3 When `ci = c(FALSE, TRUE)`, retain the bootstrap axes and emit no error
...
- [x] 5.4 Tests for the `ci = FALSE` rejection (error on bootstrap-only options) and the `ci = c(FALSE, TRUE)` non-rejection
