...
- **`ssd_min_pmix()` helper** — not added: `ssd_min_pmix` is already an `ssdtools` export (the default function users pass); a same-named ssdsims helper would shadow it. `min_pmix` naming stays internal to the constructor; any registry-style helper belongs with `min-pmix-registry`.
