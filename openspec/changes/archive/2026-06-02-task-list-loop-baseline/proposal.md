...
- **Scenario extensions (reviewer-driven, #80):** `ssd_define_scenario()` gains a validated `replace` knob (the `sample` axis reads `scenario$replace`) and now retains the validated `ssd_data()` collection in `$data` (with `$datasets` keeping the names for the targets/registry path).
