...
- Add a baseline **runner** `ssd_run_scenario_baseline(scenario)` that loops over the four derived tables with `purrr::pmap()` (sample → data → fit → hc), threading each step's output to the next by looking up the parent result via its id foreign key, and returning the collected results. The runner does **no** task expansion of its own — it consumes `ssd_scenario_tasks()` — and needs no `data` argument, reading the data the scenario retains.
- **Scenario extensions (reviewer-driven, #80):** `ssd_define_scenario()` gains a validated `replace` scenario option (the `sample` axis reads `scenario$replace`) and now retains the validated `ssd_data()` collection in `$data` (with `$datasets` keeping the names for the targets/registry path).
