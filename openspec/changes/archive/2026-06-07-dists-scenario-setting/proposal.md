...
lists `dists` among the **cross-join axes** — "the grid options that fan out over
...
- **Re-classify `dists` as a fit-level scenario setting** in the
  ...
  cross-join-axes clause (2) and name it in the scenario-settings clause (3),
  alongside `proportion`/`ci`/`samples`. The taxonomy stays binary: a scenario option is
  either an axis (in `task_axes(step)`) or a scenario setting.
- **Move `dists` in the `ssd_define_scenario()` signature** out of the fit-axis
  block to the contiguous scenario-settings block, leading it (fit setting
...
- **Keep storage step-based**: `dists` remains stored at `scenario$fit$dists`
  ...
  options but marked as a setting, not an axis.
...
- `scenario-definition`: The `Constructor arguments are grouped by role`
  ...
  (not a cross-join axis) and sits in the contiguous scenario-settings block
