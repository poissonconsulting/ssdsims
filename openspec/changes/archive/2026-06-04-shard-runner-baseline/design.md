...
### Decision: the single-core runner owns and clears its output tree (Option A)

A shard's Hive path depth/axes are a function of `partition_by`/`bundle`, and the readers glob `<step>/**/part.parquet` (depth-agnostic). So replaying a scenario with a *changed* split into the same `dir` would leave stale-granularity shards that the glob unions with the new ones — double-counting tasks, because the `<step>_id` identity is `partition_by`-independent, so one task appears in both the old coarse shard and the new fine shard. Since this single-core runner recomputes every shard anyway, it simply **owns** the output tree: it `unlink()`s each `<dir>/<step>` before writing. This guarantees coherence with no manifest or prune, and keeps `dir` predictable for the `run-serial.R` compare workflow. *Why not the per-layout root (Option D) used by the targets path?* That choice exists to preserve the `targets` cache and the `error = "null"` survivor-union across layouts; the single-core runner has no cache to preserve, so clobbering is the simpler, equivalent fix.
