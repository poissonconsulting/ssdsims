...
**Reason**: This requirement existed to manage the `ci = c(FALSE, TRUE)` axis (rejecting bootstrap-only scenario options when `ci = FALSE` was the *only* value, and steering users to `ci = c(FALSE, TRUE)` to "reduce the scenario's asymmetry"). With `ci` demoted to a scalar flag, that multi-value story no longer exists. The bootstrap-option rejection itself survives — folded into the new *"ci is a scalar flag selecting bootstrap confidence intervals"* requirement — but its enablement path becomes `ci = TRUE`, not `ci = c(FALSE, TRUE)`.
...
- **THEN** it SHALL equal `task_axes(step)`: `sample` = `dataset`, `sim`, `replace`; `fit` adds `nrow`, `rescale`, `computable`, `at_boundary_ok`, `min_pmix`, `range_shape1`, `range_shape2`; `hc` adds `nboot`, `est_method`, `ci_method`, `parametric` (`ci` is a scalar hc option, not an axis — see *"ci is a scalar flag selecting bootstrap confidence intervals"*)
