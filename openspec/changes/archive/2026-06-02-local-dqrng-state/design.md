...
- **dqrng state capture/restore may not be exact** → Resolved: `dqrng_get_state()`/`dqrng_set_state()` (>= 0.4.1) round-trip a draw sequence exactly (verified against 0.4.1). A regression test takes draws, runs a nested `local_dqrng_state()` scope, and asserts the outer stream continues byte-identically to a no-nested-scope control.
...
- ~~Exact dqrng state snapshot/restore mechanism~~ **Resolved**: use `dqrng_get_state()`/`dqrng_set_state()` (>= 0.4.1), confirmed faithful on 0.4.1.
