...
- **THEN** the `sample` slice SHALL carry the named `datasets` and `partition_by$sample`; the `fit` slice SHALL carry `fit$dists`, the `min_pmix` functions, and `partition_by` for `sample` and `fit`; and the `hc` slice SHALL carry `hc$proportion`, `hc$samples`, and `partition_by` for `fit` and `hc`
