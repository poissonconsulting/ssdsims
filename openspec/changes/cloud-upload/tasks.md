...
- [ ] 1.1 Add `AzureStor` and `AzureRMR` to `DESCRIPTION` `Suggests`; document that the upload/read path is opt-in (the DuckDB `azure` extension used by `ssd_results()` is a runtime DuckDB extension loaded via `duckdb`/`duckplyr` — `INSTALL azure; LOAD azure;` — not a package dependency)
...
- [ ] 1.4 Define the three generics (`UseMethod`): `ssd_upload_shard(path, upload)`, `ssd_test_upload(upload)`, and `ssd_results(upload, step)`; each with a default method that aborts on an unknown class
...
- [ ] 1.9 Implement `ssd_results.ssdsims_upload_azure_blob(upload, step)`: `INSTALL azure; LOAD azure;` on a DuckDB/`duckplyr` connection, resolve read credentials (fail loud naming the missing requirement if the extension or a credential is absent), and return a lazy `duckplyr` table over `<container>/<step>/**/part.parquet` read **in place** (no download); accept `step ∈ {sample, fit, hc}` and offer the uploaded `summary`
- [ ] 1.10 Implement `ssd_results.ssdsims_upload_dryrun(upload, step)`: abort with an informative error (a dry run uploads nothing — read the local shards directly), never returning an empty/local table
- [ ] 1.11 Compute and surface the cloud copy's sha256 from `ssd_upload_shard()` for the manifest to record alongside the local sha256
- [ ] 1.12 Roxygen-document all constructors and the three generics; on `?ssd_upload_shard` document the extension contract (constructor + the three generics `ssd_upload_shard()`/`ssd_test_upload()`/`ssd_results()`, no edit to existing methods); `@export` the public functions and run `devtools::document()` to update `NAMESPACE`/`man/`
...
- [ ] 4.7 Test `ssd_results()`: dry-run aborts with the "nothing uploaded" message; Azure builds the expected `<container>/<step>/**/part.parquet` glob and fails loud when the extension/credentials are absent (mock/skip the live in-place read; the row-for-row Azure round-trip is a documented manual/lab step)
...
- [ ] 5.5 Described (non-evaluated) section — verify the upload: show `ssd_results(upload, step)` reading the results back **in place** and a one-line `dplyr::count()` round-trip as the immediate post-upload test
- [ ] 5.6 "What to pay attention to" callout: credentials must reach the **workers** (controller `script_lines`/module loads or scheduler env propagation, not just the login node); the Azure client + DuckDB `azure` extension must be available on workers (ManyLinux path); a missing credential **fails loud** (`error = "null"` keeps the rest shipping); unchanged shards are not re-uploaded (content-hash skip); the read-back is **in place** (no download)
- [ ] 5.7 Add forward links to the new vignette from `vignettes/sharded-pipeline.qmd` and `vignettes/cluster-pipeline.qmd`
- [ ] 5.8 Render the vignette offline (no `AZURE_*` set) to confirm it builds with no network/credentials
