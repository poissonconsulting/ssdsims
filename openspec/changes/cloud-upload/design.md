...
- **Credential schema.** The client is decided (`AzureStor` + `AzureRMR`); what remains is which auth mode(s) `ssd_test_upload.ssdsims_upload_azure_blob()` resolves and in what precedence — `SSDSIMS_AZURE_STORAGE_ACCOUNT` + `SSDSIMS_AZURE_STORAGE_KEY` (account key, via `AzureStor`), a SAS token, or an AAD service principal (via `AzureRMR`). The spec only requires that a missing required variable is named loudly; the precise env-var set is pinned during implementation.
