...
- **Dependencies**: adds `jsonlite` (manifest JSON), `digest` (file sha256), and `sessioninfo` (complete session info) to `Imports`. (`digest` and the `ssd_file_sha256()` helper are shared with `registry`; whichever lands first introduces them.)
