...
`ssd_define_scenario()` SHALL order its arguments by role: (1) the required data/`seed`/`nsim` inputs and the dataset `name`; (2) the **structural cross-join axes** that are meaningful regardless of `ci` (`nrow`, `replace`, `rescale`, `computable`, `at_boundary_ok`, `min_pmix`, `range_shape1`, `range_shape2`); (3) the **fit-level simulation setting** `dists`; (4) the **`ci` group** — `ci` first (the gate), then the bootstrap **cross-join axes** it gates (`nboot`, `ci_method`, `parametric`), then the within-task hc **simulation settings** (`est_method`, `proportion`, `samples`); (5) the **partitioning and remaining arguments** (`partition_by`, `bundle`, `upload`). A simulation setting is any knob absent from `task_axes(step)`: it never multiplies tasks, but is consumed inside each task — fanning out within the task's output (`est_method`, `proportion`) or applied uniformly (`ci`, `dists`, `samples`). `dists` is the **fit**-level setting (a single character vector handed whole to every fit task's `ssd_fit_dists()` call, absent from `task_axes("fit")`); `ci`/`est_method`/`proportion`/`samples` are **hc**-level. The bootstrap-only knobs `nboot`/`ci_method`/`parametric` are genuine hc cross-join axes but are only meaningful when `ci = TRUE`, so they SHALL sit **after** `ci` rather than before it. Storage SHALL remain step-based: `dists` is stored at `scenario$fit$dists` and the hc knobs at `scenario$hc`, with `ci` leading: `ci`, `nboot`, `ci_method`, `parametric`, `est_method`, `proportion`, `samples`. `print.ssdsims_scenario()` SHALL render the hc grid in that same `ci`-led order (settings flagged) and render `dists` among the fit knobs, marked as a setting rather than an axis.

#### Scenario: ci leads its gated knobs in the signature
- **WHEN** the `ssd_define_scenario()` signature is inspected
- **THEN** `dists`, `ci`, `nboot`, `ci_method`, `parametric`, `est_method`, `proportion`, and `samples` SHALL appear adjacent to one another in that order, after the last structural axis (`range_shape2`) and before the partitioning arguments (`partition_by`, `bundle`, `upload`), so `ci` precedes the bootstrap-only knobs it gates
...
#### Scenario: Print leads the hc grid with ci
- **WHEN** an `ssdsims_scenario` is printed
- **THEN** the hc grid SHALL render `ci` first, then the bootstrap axes (`nboot`, `ci_method`, `parametric`), then the within-task settings (`est_method`, `proportion`, `samples`), with the non-axis knobs flagged as settings; and the fit knobs SHALL render `dists` marked as a setting
