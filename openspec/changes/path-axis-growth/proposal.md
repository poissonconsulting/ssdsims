...
- **Dependencies (direction)**: **Depends on** `task-tables` (the factory and runners it exercises) and on `task-tables` having **landed** (it has). The precise *expected-cached set* — cache-by-file-existence vs. content-hash — **finalises against the invalidation model pinned by `hive-partitioning`** (the §8 fork); the requirement is written so the contract holds either way, and the test's exact skip/build assertions are locked to that model once it lands. It is **downstream of `hive-partitioning`** for that reason. Pairs with `step-scenario-slice` (the step-axis counterpart of the same minimal-rebuild contract) and with `shard-atomic-rewrite` (the inner-axis counterpart). TARGETS-DESIGN.md §12 DAG: `task-tables → path-axis-growth`, finalised against `hive-partitioning`.
- **When to land it**: after `hive-partitioning` pins the invalidation model (so the expected-cached set is final); it gates none of the cluster work and adds no runtime dependency.
