...
The package SHALL derive an `hc` task table by crossing each fit-task identity with each row of the scenario's `hc` argument grid (`nboot`, `est_method`, `ci_method`, `parametric`). The scenario's scalar `ci` flag SHALL be applied uniformly to every hc task from the scenario (read by the runner), and SHALL NOT be emitted as a task-row column — it is not a cross-join axis and is absent from `task_axes("hc")`. When `ci = FALSE`, the derivation SHALL still canonicalise the bootstrap-only knobs (`nboot`, `ci_method`, `parametric`) to `NA` on every hc row (so they do not enter task identity for a no-bootstrap estimate), leaving `est_method` as the only fan-out axis; this canonicalisation is keyed off the scenario's scalar `ci`, not an emitted `ci` column. When `ci = TRUE`, the grid SHALL fan out over `nboot × est_method × ci_method × parametric`.
...
- **THEN** the hc task table SHALL have `K * H` rows, each carrying its parent fit-task identity columns and the hc-grid argument columns, and SHALL NOT carry a `ci` column

#### Scenario: ci = FALSE leaves bootstrap-only knobs NA
- **WHEN** the hc task table is derived from a scenario with the scalar `ci = FALSE` and multiple `est_method` values
- **THEN** the expansion SHALL produce one row per `est_method` with `nboot`/`ci_method`/`parametric` set to `NA`, rather than fanning out across those bootstrap-only knobs, with no `ci` column emitted
...
- **THEN** the expansion SHALL fan out over `nboot × est_method × ci_method × parametric`, and `ci` SHALL be neither a per-task identity axis (`task_axes("hc")`) nor an emitted column
