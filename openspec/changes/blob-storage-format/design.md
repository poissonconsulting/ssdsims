...
### Decision: evaluate three candidate encodings against the three constraints
...
The leading choice is **(a) binary `serialize(ascii = FALSE)` carried as base64 text in the existing `VARCHAR` `fit_blob` column** — it is lossless (byte-identity oracle holds), strictly smaller and cheaper than ASCII serialisation, needs no change to the column *type* (still a string column, so duckplyr is happy and the column is projectable out exactly as today), and touches only `encode_obj()`/`decode_obj()`. The one open question is the **size/CPU break-even and the base64 dependency**: ASCII `serialize` is itself fairly compact, so before locking (a) in, the implementation SHALL benchmark `encode_obj`/`decode_obj` size and time on a representative `fitdists` (ASCII-`VARCHAR` vs base64-binary), and choose the base64 codec (a small dependency vs a vendored helper) on that basis. If the benchmark shows binary+base64 does **not** beat ASCII on the realistic fit sizes (base64's 4/3 expansion eating the binary halving), the encoding stays as-is and this change instead just records the decision and tightens the spec contract — the swap is only landed if it wins. This is recorded as a benchmark-gated decision rather than an unconditional swap.
...
- **base64 codec / dependency.** Whether to take a small base64 dependency or vendor a helper, decided by the benchmark in the chosen-format decision.
