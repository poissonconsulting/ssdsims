test_that("ssd_generate_data.function works", {
  withr::local_seed(42)
  data <- ssd_generate_data(ssdtools::ssd_rlnorm, nrow = 5, nsims = 10)
  expect_snapshot_data(data, "function")
})

test_that("ssd_generate_data.function works multiple nrow", {
  withr::local_seed(42)
  data <- ssd_generate_data(ssdtools::ssd_rlnorm, nrow = c(5,7), nsims = 2)
  expect_snapshot_data(data, "function2")
})

test_that("ssd_generate_data.character works", {
  withr::local_seed(42)
  data <- ssd_generate_data("lnorm", nrow = 5, nsims = 10)
...
test_that("ssd_generate_data.fitdists works top", {
  ...
  withr::local_seed(42)
  data <- ssd_generate_data(fit, dist = "lnorm", nrow = 5, nsims = 10)
  expect_snapshot_data(data, "fitdists_top")
})

test_that("ssd_generate_data.fitdists works top multiple", {
  data <- ssddata::ccme_boron
  fit <- ssdtools::ssd_fit_dists(data, dists = c("lnorm", "gamma"))
  withr::local_seed(42)
  data <- ssd_generate_data(fit, dist = "top", nrow = c(5,7), nsims = 2)
...
test_that("ssd_generate_data.fitdists works name", {
  ...
  fit <- ssdtools::ssd_fit_dists(data, dists = c("lnorm", "gamma"))
  withr::local_seed(42)
  data <- ssd_generate_data(fit, dist = "gamma", nrow = 5, nsims = 10)
...
test_that("ssd_generate_data.tmbfit works", {
  ...
  withr::local_seed(42)
  data <- ssd_generate_data(tmbfit, nrow = 5, nsims = 10)
