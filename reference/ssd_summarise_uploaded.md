...
`<container>[/<prefix>]/<step>/**/part.parquet` Hive glob via DuckDB's
`azure` extension - resolving the **same** front-end secret as the write
path and remapping it (with the account derived from `url`) into a
DuckDB `azure` secret - and returns the union as a lazy `duckplyr`
tibble (not collected, so the read and projection stay in DuckDB). By
default it projects away the heavy `dists`/`samples` list-columns (the
analysis-ready summary, mirroring
[`ssd_summarise()`](https://poissonconsulting.github.io/ssdsims/reference/ssd_summarise.md));
pass `drop_samples = FALSE` to keep them when the in-flight bootstrap
`samples` are needed. The default method (an unknown destination) and
the dry-run method both abort.
