...
Credentials stay **external**: the Azure methods resolve them from the
environment (`AZURE_STORAGE_ACCOUNT` plus one of `AZURE_STORAGE_KEY`,
`AZURE_STORAGE_SAS`, or the service-principal trio) at call time, and the object
...
The factory runs the `ssd_test_upload()` probe **once up front**, then pairs each
step shard with a no-op `upload_<step>` target:
...
aborting loudly (naming the missing `AZURE_*` variable) if your wiring is wrong:

```{r}
#| label: azure-preflight
#| eval: false
Sys.setenv(AZURE_STORAGE_ACCOUNT = "<account>", AZURE_STORAGE_KEY = "<key>")
...
`AZURE_*` credentials into a DuckDB `azure` secret for you). Because it is lazy
...
- **Credentials must reach the *workers*.** `ssd_test_upload()` is easy to run
  ...
  nodes**. Set `AZURE_*` so the workers see them --- via the controller's
