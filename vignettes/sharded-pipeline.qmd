...
`partition_by` is a free re-layout, `run-serial.R` finishes by reading back
`results-serial/summary.parquet` and `results/summary.parquet` and asserting the
per-task estimates are byte-identical. Whatever the driver --- baseline,
