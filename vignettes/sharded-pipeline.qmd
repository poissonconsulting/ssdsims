...
`targets` caches, invalidates, and reruns each shard independently. A ready
template ships with the package:

```{r}
#| label: template-path
system.file("targets-templates", "local", "_targets.R", package = "ssdsims")
```

Copy it to your project root, edit the scenario, then run the pipeline (this
needs the `targets` and `tarchetypes` Suggests):

The template ships with a companion `run.R` driver (`tar_make()` + read the
summary). Copy both to your project root and run it:
...
file.copy(file.path(dir, c("_targets.R", "run.R")), ".")
source("run.R") # tar_make(): one target per shard -> results/<step>/...; then the summary
# or, from a shell:  Rscript run.R
```

The template uses `format = "file"` (targets passes the shard *path*, not its
...
Whatever the driver --- baseline, single-core shards, or `targets` --- the
per-task results are identical; choose the one that fits the run.
