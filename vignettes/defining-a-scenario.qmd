...
unsized or irreproducible generation is impossible. Each generator draws on an
independent dqrng stream keyed by its dataset *name* (with `.seed` as the base
seed), so one `.seed` reproducibly fans out across all generators in a call;
the global RNG state is untouched. Generation requires the suggested `dqrng`
package to be loaded (`library(dqrng)`) --- ssdsims never loads it implicitly.
...
The scalar `ci` is carried uniformly on the `hc` table (it is not an axis), as
is the `est_method` setting. With `ci = TRUE` the bootstrap knobs (`nboot`,
...
`local_dqrng_backend()` scope (which requires the suggested `dqrng` package to
be loaded --- hence the `library(dqrng)` at the top of this vignette) and seeds
each task exactly once from `scenario$seed` plus a per-task primer
(`task_primer()` over the task's identity), so two runs of a scenario with a
fixed `seed` yield identical results, regardless of the order in which tasks
run:
