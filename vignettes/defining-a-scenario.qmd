...
declarative arguments --- a seed, the number of simulations, the sample sizes,
the dataset *names*, and the fit/hc argument grids. It draws **no** random numbers, expands
...
no default because changing it fully re-roots every draw). Everything else is
a scenario option with a sensible default.
...
The `min_pmix` option is referenced **by name**: the name --- not the function
...
is the `est_method` setting. With `ci = TRUE` the bootstrap axes (`nboot`,
...
scenario options (every requested `est_method` is summarised within each task from a single
