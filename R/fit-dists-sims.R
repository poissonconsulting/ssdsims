...
#' @export
ssd_fit_dists_sims <- function(sims, dists = ssdtools::ssd_dists_bcanz(), ..., silent = TRUE, save_to = NULL, return_items = TRUE, seed = NULL, stream = 1L, start_seed = 1L) {
  chk::chk_null_or(save_to, vld = chk::vld_dir)
  chk::chk_flag(return_items)
  chk::chk_list(sims)
  if(!chk::vld_all(sims, chk::vld_data)) {
    chk::chk_all(sims, chk::chk_data)
  }
  if(!length(sims)) {
    return(list())
  }
  ...
  n <- length(sims)
  seeds <- get_lecuyer_cmrg_seed_stream(seed = seed, nseeds = n, start_stream = stream, start_seed = start_seed)
