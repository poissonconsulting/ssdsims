...
ssd_sim_data.data.frame <- function(
  ...
  if (nrow(data) == nsim) {
    states <- get_lecuyer_cmrg_seeds_stream(
      seed = seed,
      nsim = nsim,
      start_sim = start_sim,
      stream = stream
    ...
    data <- purrr::map(
      states,
      \(state) slice_sample_seed(x, n = nrow, replace = replace, state = state),
      .progress = .progress
    ) |>
      purrr::map2(sims, \(.x, .y) {
        dplyr::mutate(.x, sim = .y, stream = stream)
      }) |>
      dplyr::bind_rows() |>
