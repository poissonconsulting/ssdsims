...
- ❗️ \[scenario-input-types\] Extend
  ...
  string), materialised once in the constructor under the dqrng-only
  `ssd_data(..., .seed = NULL)` contract. Unblocked (deps `task-primer`
  / `local-dqrng-state` landed); gates `migrate-public-api`.
