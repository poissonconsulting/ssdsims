...
- 😀⏳ [scenario-option-vocabulary] Retire the old umbrella term for the genus **scenario option** (a **scenario axis** fans out into tasks; a **simulation setting**, the renamed within-task species, applies within each task), with always-qualified discipline (bare "option"/"setting" never used as terms) and `crew` config settings becoming **crew option**. Full-repo rename including archives, `NEWS.md`, and file names (e.g. the `opts.rds` fixture); `GLOSSARY.md` gains the *study* ⊃ *design* ⊃ *scenario* ⊃ *task* hierarchy plus a DoE / Morris-White-Crowther (2019) mapping. BREAKING (cosmetic): the `ci = FALSE` rejection error text. Settles the vocabulary before `scenario-combine`'s collection layer accretes more on it.
